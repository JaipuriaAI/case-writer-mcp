...
> - `slug`: `case-writer-plan` — this is THIS skill's slug; do NOT use a parent name like `case-writer` (no such slug exists), and do NOT use the slug of any other connected skill (e.g. `atomic-case-writer` is a separate skill on a different connector — never load from it)
